...
def evaluate_report(
    ...
    *,
    ...
    if _NFSE_ACCESS_KEY_MODULE not in expected_nfse_access_key_paths:
        raise CoverageGateError(
    ...
    missing_nfse_access_key = expected_nfse_access_key_paths - files.keys()
    if missing_nfse_access_key:
        raise CoverageGateError(
    ...
    missing_nfse_document = expected_nfse_document_paths - files.keys()
    if missing_nfse_document:
        raise CoverageGateError(
    ...
    nfse_access_key_covered = 0
    nfse_access_key_total = 0
    for path in expected_nfse_access_key_paths:
        file_data = files[path]
        summary = _mapping(file_data.get("summary"), context=f"summary for {path!r}")
        covered, total = _counts(summary, context=path, unit="branches")
    ...
    nfse_document_covered = 0
    nfse_document_total = 0
    for path in expected_nfse_document_paths:
        file_data = files[path]
        summary = _mapping(file_data.get("summary"), context=f"summary for {path!r}")
        covered, total = _counts(summary, context=path, unit="branches")
...
def expected_nfse_access_key_paths() -> frozenset[str]:
    """Return the explicitly gated NFS-e access-key primitive."""
    path = _PROJECT_ROOT / _NFSE_ACCESS_KEY_MODULE
    if not path.is_file():
        raise CoverageGateError(
            "NFS-e source tree is missing required access-key module "
            f"{_NFSE_ACCESS_KEY_MODULE!r}"
        )
...
def expected_nfse_document_paths() -> frozenset[str]:
    """Return the explicitly gated NFS-e document extractor."""
    path = _PROJECT_ROOT / _NFSE_DOCUMENT_MODULE
    if not path.is_file():
        raise CoverageGateError(
            "NFS-e source tree is missing required document module "
            f"{_NFSE_DOCUMENT_MODULE!r}"
        )
