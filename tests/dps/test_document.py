"""Tests for public unsigned-DPS structural inspection."""
...
from nfse_br.dps import document as document_module
...
def _reject(xml_bytes: object, code: str) -> DpsDocumentError:
...
@pytest.mark.parametrize("code", [None, "not-a-controlled-code"])
def test_rejects_uncontrolled_error_codes_without_echoing_them(code: object) -> None:
    with pytest.raises(ValueError) as caught:
        DpsDocumentError(cast(str, code))

    assert str(code) not in str(caught.value)


def test_accepts_builder_output_and_returns_derived_identity() -> None:
    identity = inspect_unsigned_dps(_xml())

    assert type(identity) is DpsIdentity
    assert identity.value == _EXPECTED_ID
...
def test_accepts_lexical_series_alias_and_distinct_municipalities() -> None:
    ...
    regular_identity = inspect_unsigned_dps(regular)
    alias_identity = inspect_unsigned_dps(leading_zero)
    assert regular != leading_zero
    assert regular_identity.value == alias_identity.value
    assert regular_identity.series.value == "123"
    assert alias_identity.series.value == "00123"
...
def test_escaped_signature_like_text_is_not_an_element() -> None:
    ...
    assert inspect_unsigned_dps(xml_bytes).value == _EXPECTED_ID
...
def test_rejects_document_over_local_limit_before_parsing() -> None:
    _reject(b" " * (document_module._MAX_XML_BYTES + 1), "document_too_large")
...
def test_valid_invalid_valid_calls_are_independent() -> None:
    ...
    assert inspect_unsigned_dps(valid).value == _EXPECTED_ID
